...
describe("test adding and voting on restos", () => {
  const userId = "123";
  const name = "Adrian";
  ...
  const restaurant4Id = 'do you sand';
  const restaurant5Id = 'choppawontmissayuh'
  ...
  // restaurants 4 and 5 already have some votes just so we can test prepareResults
  const restaurant4 = {
    id: restaurant4Id,
  ...
  const restaurant5 = {
    ...
    votes: 0,
    user_ratings_total: 766,
    reviews: ["top 10 songs in all of humanity"]
  };
  ...
  test("1. setup restaurants that have been voted for", async () => {
    await new Promise<void>((resolve) => {
    ...
    test("3. test end voting", async () => {
      await new Promise<void>((resolve) => {
        clientSocket.emit("room:endVoting");
        
        clientSocket.once("syncState", (room) => {
          expect(room.users).toContainEqual({ userId, name, userState: "FINISHED" });
          expect(room.users).toContainEqual({ userId: userId1, name: name1, userState: "FINISHED" });
          expect(room.gameState).toStrictEqual("FINISHED");
          resolve();
        });     
      });
    });

    test("4. test prepareResults", async () => {
      clientSocket.emit("room:prepareResults");

      // dawg... (skull emoji)
      clientSocket.once("syncState", (room) => {
        expect(room.restaurants[0].votes).toBeGreaterThanOrEqual(room.restaurants[1].votes)
        expect(room.restaurants[1].votes).toBeGreaterThanOrEqual(room.restaurants[2].votes)
        expect(room.restaurants[2].votes).toBeGreaterThanOrEqual(room.restaurants[3].votes)
        expect(room.restaurants[3].votes).toBeGreaterThanOrEqual(room.restaurants[4].votes)
      });
    });
});